...
Three images are exceptions and *are* committed here: `assets/valorant-logo.png`, the borderless cut of it that fills the small-icon slot, and the dimmed idle variant of that. They are hotlinked out of this repository the way league-rpc hotlinks its own, and they are the fallback when no Riot art applies. `TestLogoURLsPointAtCommittedAssets` proves the files exist in the tree, and `TestRepoHostedImagesResolve` proves the URLs actually serve an image. `TestCommittedLogosAreFullyOpaque` proves they fill their canvas: Discord draws them on the viewer's own background, so a transparent edge reads as a notch cut out of the icon.
