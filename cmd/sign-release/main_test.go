...
func TestRun_WritesSumsAndSignature(t *testing.T) {
	...
	dir := t.TempDir()
	artifact := filepath.Join(dir, "valorant-rpc-gui.exe")
	...
	wantLine := hex.EncodeToString(wantDigest[:]) + "  valorant-rpc-gui.exe\n"
	...
	if len(fields) != 2 || fields[1] != "valorant-rpc-gui.exe" {
		t.Fatalf("SHA256SUMS.sig = %q, want '<hex>  valorant-rpc-gui.exe'", sig)
	}
