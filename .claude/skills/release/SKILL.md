...
3. **Check the config schema.** If this release changes the config tree, it
   needs a registered migration step in `internal/config/migrate.go` before
   it ships. Once this app has public users, a schema change without a
   migration corrupts their settings. Never propose telling a user to delete
   `%APPDATA%\valorant-rpc\config.json`.

4. **Draft the release notes.** Follow the `rpc-release-notes` skill's process
...
5. **Present and stop.** Show the user the proposed version number and the
...
6. **Commit, once approved.** Stage what belongs in the release and commit
...
8. **Ask before tagging.** Once master is pushed, ask the user whether to
   ...
   GitHub release on its own. A tag containing a hyphen publishes as a
   prerelease.

9. **Do not watch the workflow run.** The user monitors the GitHub Actions
...
## This repo is not league-rpc

Same shape, different identity. Before a first release, confirm the release
pipeline points at this repo and not the one it was copied from:
`internal/updates` `RepoSlug`, the embedded update public key, the NSIS
product and upgrade codes, and the `release-signing` environment on
`Its-Haze/valorant-rpc`. The signing keypair is this repo's own and is not
shared with league-rpc.
