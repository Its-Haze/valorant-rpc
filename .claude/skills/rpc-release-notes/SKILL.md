...
description: Draft the curated changes file for a Valorant RPC release (.github/release-notes/vX.Y.Z.md). Use when cutting a release, preparing to tag a version, or asked to write/draft release notes or a changelog for this project.
---

# Valorant RPC release notes
...
- Agent, map and rank art is hotlinked from valorant-api.com. If a release
  changes which images are used, that is user-visible and belongs in the
  notes.
- The agent lookup is the only feature that talks to Riot's servers. Any
  change to when it runs, what it reads, or how it fails belongs in the
  notes in plain language, not buried as an internal detail.
- Presence contexts are user-facing vocabulary. Call them what the Display
  screen calls them (in client, in queue, custom game, agent select, in
  match), not by their internal `sessionLoopState` names.
