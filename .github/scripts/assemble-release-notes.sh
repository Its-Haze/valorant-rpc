...
installer_display="valorant-rpc-setup.exe"
...
Download and run **[$installer_display]($installer_url)** to install Valorant RPC.

Already have it installed? You don't need this. Valorant RPC checks for updates on its own, and you can install them right from the About screen.
...
3. Valorant RPC starts automatically and lives in your system tray from then on. Click the tray icon any time to open the window.
...
Valorant RPC is open-source and not yet signed by a certificate authority, so
