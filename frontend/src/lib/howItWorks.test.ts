...
describe("howItWorksCopy", () => {
  it("always describes the same four stages", () => {
    for (const inputs of COMBINATIONS) {
      ...
      expect(steps.map((s) => s.title)).toEqual([
        "It starts",
        "It waits",
        "Valorant opens, it connects",
        "Valorant closes, it sleeps",
      ]);
  ...
  it("says the user starts it, at any time, when launch at startup is off", () => {
    ...
    expect(steps[0].body).toContain("You start Valorant RPC yourself");
    expect(steps[0].body).toContain("mid-match");
  });
