...
// GameDetector reports whether Valorant's own process is running.
// *RiotSupervisor satisfies this.
type GameDetector interface {
	GameRunning() bool
}

// DiscordSupervisor wraps a Supervisor for the Discord IPC connection, also
// gated on Valorant's own process via game. See ADR-0003.
...
// newDiscordSupervisor builds a DiscordSupervisor; game gates connect/stay-connected.
...
func (c *discordGatedConnector) IsConnected() bool {
	return c.Connector.IsConnected() && c.ds.processRunning() && c.ds.game.GameRunning()
}
