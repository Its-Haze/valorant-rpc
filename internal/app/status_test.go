...
type fakeConns struct {
	discord, riot, game, stalled atomic.Bool
}

func (f *fakeConns) DiscordConnected() bool { return f.discord.Load() }
func (f *fakeConns) RiotConnected() bool    { return f.riot.Load() }
func (f *fakeConns) GameRunning() bool      { return f.game.Load() }
func (f *fakeConns) PresenceStalled() bool  { return f.stalled.Load() }
...
func TestStatusBridge_SnapshotAssemblesFromEverySource(t *testing.T) {
	...
	conns.game.Store(true)
	probe := &fakeProbe{}
	probe.set(discord.LastSent{Data: &discord.RPCData{Details: "Competitive", State: "In a match"}})
	pauser := &fakePauser{paused: true}

	b := newStatusBridge(conns, probe, pauser, nil)
	b.context = string(types.ContextInMatch)
	...
	want := StatusSnapshot{
		...
		Paused:           true,
		Context:          "in-match",
		Presence:         &discord.RPCData{Details: "Competitive", State: "In a match"},
	}
	if !got.equal(want) {
...
func TestStatusBridge_RunPicksUpContextFromStateChannel(t *testing.T) {
	conns := &fakeConns{}
	...
	st := state.NewState()
	st.SessionLoopState = types.SessionLoopPregame
	states <- st

	waitForSnapshot(t, got, func(s StatusSnapshot) bool {
		return s.Context == string(types.ContextAgentSelect)
	})
}
...
func TestApp_GetStatusReflectsSourcesAndZeroWithoutWiring(t *testing.T) {
	...
	conns := &fakeConns{}
	conns.riot.Store(true)
	wired := New(store, &fakePauser{paused: true},
		WithStatus(conns, &fakeProbe{}, nil))

	got := wired.GetStatus()
	if !got.RiotConnected || !got.Paused {
		t.Fatalf("GetStatus = %+v, want RiotConnected and Paused set", got)
	}
}
