...
func TestApp_RenderTemplatePreview_UsesSampleDataAndDefaults(t *testing.T) {
	...
	dt, st := config.DefaultConfig().Presence.Templates["in-match"].Details, config.DefaultConfig().Presence.Templates["in-match"].State
	got, err := a.RenderTemplatePreview("in-match", config.TemplatePair{Details: dt, State: st}, nil)
	if err != nil {
...
func TestApp_RenderTemplatePreview_ReportsUnknownTokens(t *testing.T) {
	...
	got, err := a.RenderTemplatePreview("in-client",
		config.TemplatePair{Details: "{riot_id} {foo}", State: "{bar}"},
		map[string]string{"riot_id": "Haze"})
	if err != nil {
		t.Fatalf("RenderTemplatePreview: %v", err)
	}
	if got.Details != "Haze {foo}" || got.State != "{bar}" {
...
func TestApp_RenderTemplatePreview_BlankLineMatchesRuntimeDefault(t *testing.T) {
	...
	got, err := a.RenderTemplatePreview("in-match", config.TemplatePair{Details: "", State: "custom state"}, nil)
	if err != nil {
		t.Fatalf("RenderTemplatePreview: %v", err)
	}
...
func TestApp_RenderTemplatePreview_EmptyMapUsesSampleData(t *testing.T) {
	...
	got, err := a.RenderTemplatePreview("agent-select", config.TemplatePair{}, map[string]string{})
	if err != nil {
		t.Fatalf("RenderTemplatePreview: %v", err)
	}
	if got.Details == "" {
...
func TestApp_GetDisplayPreview_ShowsScoreWhenStatsEnabled(t *testing.T) {
	...
	if !strings.Contains(got.State, "7-5") {
		t.Fatalf("State = %q, want the sample score included", got.State)
	}
}
...
func TestApp_GetDisplayPreview_HidesScoreWhenStatsDisabled(t *testing.T) {
	...
	if strings.Contains(got.State, "7-5") {
		t.Fatalf("State = %q, the score should be hidden", got.State)
	}
}
...
func TestApp_GetDisplayPreview_HidesRankWhenDisabled(t *testing.T) {
	...
	if strings.Contains(got.State, "Immortal 2") {
		t.Fatalf("State = %q, the rank should be hidden", got.State)
	}
}
...
// Without a catalogue the preview still has to show something, and the app's
// own icon is what a real send falls back to as well.
...
// The emblem only replaces the small icon where a real ranked send would
// swap it in, which is the three contexts whose sample sits in Competitive.
func TestApp_GetDisplayPreview_UsesTheRankEmblemWhereRankedSendsWould(t *testing.T) {
	cat := fakeCatalogue{testCatalogue(t)}
	a := New(config.NewStore(config.DefaultConfig()), &fakePauser{}, WithCatalogue(cat))

	emblem, ok := cat.Snapshot().Tier(previewSampleTier, types.DefaultLocale)
	if !ok {
		t.Fatalf("tier %d did not resolve in the test catalogue", previewSampleTier)
	}

	for ctx, wantEmblem := range map[string]bool{
		...
		if got.SmallImage != want {
			t.Errorf("%s small image = %q, want %q", ctx, got.SmallImage, want)
		}
	}
}

// Turning the rank off has to drop the emblem too, not just the text.
...
func TestApp_GetTemplateTokens(t *testing.T) {
	...
	// No rank: in client the tier follows a queue the player did not choose.
	want := []string{"mode", "party", "party_size", "max_party_size", "riot_id", "account_level", "availability", "idle"}
...
// GetDisplayPreview blanks tokens in the sample map to honor the display
// toggles. template.SampleData hands back a clone for exactly this reason; if
// it ever stopped, one preview with a toggle off would blank every later one.
