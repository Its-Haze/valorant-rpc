...
// Default heartbeat/reclaim cadence, carried over from league-rpc, where
// League's own presence was observed outlasting a 5s heartbeat.
const (
...
// Config returns the live config, so a caller holding only the Updater can
// read a setting without also being handed the Store.
func (u *Updater) Config() *config.Config {
	return u.store.Load()
}

// Run drives the heartbeat/reclaim ticker until ctx is canceled. Daemon
// starts this alongside the Discord and Riot Connection Supervisors.
...
func (u *Updater) DelayUpdate(newState *state.State) {
	...
	u.logger.Debug().
		Dur("delay", delay).
		Str("context", string(newState.PhaseContext())).
